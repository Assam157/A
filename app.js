...
app.post("/RegisterPage",async function(req,res){
    const { username, password } = req.body;
...
app.post(
    ...
    async (req, res) => {
      try {
        ...
        await newHR.save();
        const mailOptions = {
            from: "maitreyaguptaa@gmail.com", // Your email
            to: "maitreyaguptaa@gmail.com", // The email you want to send to
            subject: "Aadhar and PAN Card Files from New HR Signup",
            text: `
              A new HR has registered with the following details:
              
              Name: ${newHR.name}
              Contact: ${newHR.contact}
              
              Attached are the Aadhar and PAN card files submitted during signup.
            `,
            attachments: [
              {
                filename: "AadharCard." + aadharCard.mimetype.split("/")[1],
                content: aadharCard.buffer,
              },
              {
                filename: "PANCard." + PANCard.mimetype.split("/")[1],
                content: PANCard.buffer,
              },
            ],
          };
...
app.get('/dashboard', async (req, res) => {
    try {
        ...
        const hrs = await HRModel.find({},{name:1,contact:2});
        const jobs=await JobModel.find({},{name:1,description:2,immidiateHiring:3,vacancy:4})
        const internships=await InternshipModel.find({},{name:1,description:2,immidiateHiring:3,vacancy:4})
        res.status(200).json({ applications, users, hrs,jobs,internships });
    } catch (err) {
        res.status(500).json({ message: 'Error fetching data', error: err.message });
    }
});

app.delete('/dashboard/application/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await ApplicationModel.findByIdAndDelete(id);
        res.status(200).send({ message: 'Application deleted successfully' });
    } catch (err) {
        res.status(500).send({ message: 'Error deleting application', error: err.message });
    }
});
...
// Delete HR by ID
app.delete('/dashboard/hr/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await HRModel.findByIdAndDelete(id);
        res.status(200).send({ message: 'HR deleted successfully' });
    } catch (err) {
        res.status(500).send({ message: 'Error deleting HR', error: err.message });
    }
});
app.delete('/dashboard/jobs/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await JobModel.findByIdAndDelete(id);
        res.status(200).send({ message: 'Job deleted successfully' });
    } catch (err) {
        res.status(500).send({ message: 'Error deleting Job', error: err.message });
    }
});
app.delete('/dashboard/internships/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await InternshipModel.findByIdAndDelete(id);
        res.status(200).send({ message: 'Internship deleted successfully' });
    } catch (err) {
        res.status(500).send({ message: 'Error deleting Internship', error: err.message });
    }
});
